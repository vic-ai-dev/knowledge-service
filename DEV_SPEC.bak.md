...
flowchart LR
    ...
    EMBED --> UPSERT["Upsert<br/>pgvector + PostgreSQL + rank_bm25"]
```
...
- **BM25 Index**：基于 rank_bm25（Python 内存库）的关键词倒排索引，构建于应用层内存，首次 query 时自动加载
...
- `delete_document()` — 跨存储协调删除（pgvector + rank_bm25 + Image + PostgreSQL）
...
| 数据库 | `knowledge_rag` |
| 用途 | 全文检索 + BM25 倒排索引 |
| 扩展 | `rank_bm25`（提供 BM25 索引与全文检索能力，基于 rank_bm25（Python 内存库），jieba 分词，支持中英文混合关键词精确匹配） |
...
```sql
-- rank_bm25（Python 内存库）
-- BM25 检索由 rank_bm25（Python 内存库）+ jieba 分词在应用层实现
CREATE EXTENSION IF NOT EXISTS pg_search;

-- BM25 全文搜索索引（基于 rank_bm25 内存库，Python 应用层实现）
...
Query 链路（含 LLM 回答）延迟拆解（目标 < 10s）：
  ...
  Sparse Retrieval    ~200ms     rank_bm25 BM25 查询
...
graph TB
    ...
    subgraph Storage["Storage Layer"]
        ...
        BM25["rank_bm25 (Python 内存) -- BM25 全文检索 + BM25 索引"]
    end
...
| `api/documents.py` | 数据浏览 | 封装 pgvector / rank_bm25 / ImageStorage 读取 |
...
| `document_manager.py` | 文档生命周期 | list/delete/stats，跨存储协调（pgvector + rank_bm25 + Image + PostgreSQL） |
...
# ===== 稀疏检索配置 =====
sparse_search:
  backend: rank_bm25              # rank_bm25 | elasticsearch
  host: localhost
  port: 5432
  user: postgres
  password: root123456
...
# ===== 检索配置 =====
retrieval:
  sparse_backend: rank_bm25        # rank_bm25 | elasticsearch
...
| A4 | PostgreSQL + pgvector + tsvector/GIN 数据库初始化 | ✅ | 2026-06-14 | init_knowledge_db.sql + init_knowledge_rag_db.sql；BM25 评分在应用层实现（rank_bm25 + jieba 分词，详见 T11 索引重建策略） |
...
| C11 | BM25Indexer（rank_bm25 内存检索） | ✅ | 2026-06-14 | |
...
| H6 | 一键评估脚本（eval_oneclick.py） | ✅ | 2026-06-17 | golden dataset + query_traces 双模式，加载黄金测试集批量运行 → ragas 评估 → DB 持久化 → 终端摘要表格 |
...
| T11 | BM25 索引重建与预热策略 | 中 | rank_bm25 全量重建机制：大量写入后自动脏标记重建；可选启动预热（startup warm-up）避免首次 query 冷启动延迟 |
